...
export default async function PageAjouterPlat({ params }: { params: Promise<{ debut: string }> }) {
  ...
  const [recettes, noms] = await Promise.all([repertoire(), nomsConserves(ctx.foyer.id, "plat_libre")]);
    ...
    <>
      ...
      <ChoixPlats
        ...
        tuiles={recettes
          ...
          .map((r) => ({ id: r.id, nom: r.nom, duree: r.duree, nbArticles: r.ingredients.length }))}
